...
#
# WHY? scipy has a gaussian smooth implementation which works well but that implementation is different, it does not use 1st degree B-spline interpolation. When I discovered the difference, I could not find python implementations so I ported the matlab code. 
#
...
def smooth1(P,s,output,edge_pres=False,dtype=np.float32):
    ...
    #   function to manipulate only was is necessary.
    # 
    # P.header['descrip'] 
...
def spm_smooth(P,s,output=None,edge_pres=False,dtype=np.float64):
    ...
    '''
    P            Image or path to image
    s            Smoothing kernel (list or number)
    output       Path or null. If null, return image. 
    edge_pres    Preserve edges yes/no
    '''
